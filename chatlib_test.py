...
def check_build(input_cmd, input_data, expected_output):
    ...
    try:
        output = chatlib_skeleton.build_message(input_cmd, input_data)
...
def check_parse(msg_str, expected_output):
    ...
    try:
        output = chatlib_skeleton.parse_message(msg_str)
    except Exception as e:
        output = "Exception raised: " + str(e)
...
def main():
    ...
    # msg too long
    check_build("A", "A" * (chatlib_skeleton.MAX_DATA_LENGTH + 1), None)
